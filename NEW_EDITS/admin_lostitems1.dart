...
class _AdminDashboardScreenState extends State<AdminDashboard1LostItems> {
  ...
  void _resolveReport(Report report) async {
    ...
    try {
      ...
      // 2. Immediately update the local state
      setState(() {
      ...
      // 4. OPTIONAL: Refresh all data from database to ensure consistency
      // Uncomment if you want to force a full refresh:
       _fetchItems();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to resolve report. Error: ${e.toString()}'),
          backgroundColor: Colors.red.shade600,
        ),
      );
    }
