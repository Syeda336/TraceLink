...
class NotificationsService {
  ...
  static Future<void> addNotification({
    ...
    required String
    type, // 'warning', 'message', 'claim', 'emergency', 'item_update'
    ...
    _subscription = _firestore
        ...
        .listen((snapshot) async {
          ...
          if (diff < 10) {
            ...
            final settings =
                await FirebaseService.getPrivacySettings(); // Or getNotificationSettings()
  ...
  static Future<void> sendClaimNotification({
    ...
    await addNotification(
      ...
      type: 'claim',
      data: {'itemTitle': itemTitle, 'status': claimStatus, 'screen': 'claims'},
    );
  }
  ...
  static Future<void> sendEmergencyNotification({
    ...
    await addNotification(
      ...
      type: 'emergency',
      data: {'screen': 'emergency_alerts'},
    );
  }

  // --- NEW: Local Pop-up Notification Wrapper for Emergency Alerts ---
  // This uses the showLocalNotification helper from FirebaseService for the pop-up.
  static Future<void> showEmergencyNotificationPopUp({
    required int id,
    required String title,
    required String body,
  }) async {
    // The FirebaseService helper expects a String ID
    await FirebaseService.showLocalNotification(
      id: id.toString(), // Use the alert's ID/hash as a unique String ID
      title: title,
      body: body,
