                          ...
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (context) => UserContactedSplash(
                                ...
                                onFinishNavigation: onResolve, // Pass the callback
                              ),
                            ),
                          );
                        },
...
class _AdminDashboardScreenState extends State<AdminDashboard1LostItems> {
  ...
  List<Report> reports = [
    const Report(
      ...
      date: 'Oct 14, 2025',
      status: 'Pending',
      misconductDetail: 'Finder is not responding to messages and refusing to return the item',
      itemId: 'keys101',
    ),
    const Report(
      title: 'Student ID Card',
  ...
  // ===========================================================================
  // NOTIFICATION METHODS - ADD THESE TO YOUR STATE CLASS
  // ===========================================================================
  ...
  void _warnUser(Report report) async {
    ...
    if (targetUserId != null && targetUserId.isNotEmpty) {
      ...
      await NotificationsService.sendWarningNotification(
        ...
        adminMessage: 'Please return the item immediately to avoid further action.',
      );
  ...
  void _banUser(Report report) async {
    ...
    if (targetUserId != null && targetUserId.isNotEmpty) {
      ...
      await NotificationsService.addNotification(
        ...
        message: 'Your account has been temporarily suspended due to misconduct.',
        type: 'warning',
        data: {
  ...
  void _showDeleteConfirmation(Report report) async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete the report for "${report.title}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            ...
            onPressed: () async {
              ...
              if (targetUserId != null) {
                await NotificationsService.addNotification(
                  ...
                  message: 'The report regarding "${report.title}" has been resolved by admin.',
                  type: 'item_update',
                  data: {
                    'itemTitle': report.title,
                    'status': 'resolved',
  ...
  // ===========================================================================
  // EXISTING METHODS (KEEP THESE BUT THEY NOW USE NOTIFICATIONS)
  // ===========================================================================
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedTab == 'Lost') {
      ...
      // NEW: REPORTS TAB CONTENT LIST WITH NOTIFICATION CALLBACKS
      contentList = reports.map((report) {
        return _ReportCard(
          ...
          onWarnUser: _warnUser, // Pass the warn user method
          onContactUser: _contactUser, // Pass the contact user method
        );
      }).toList();
    } else {
      // Fallback
...
// NEW CARD WIDGET FOR MISCONDUCT REPORTS (UPDATED WITH NOTIFICATION CALLBACKS)
class _ReportCard extends StatelessWidget {
  final Report report;
  final VoidCallback onDelete;
  final VoidCallback onViewDetail;
  final Function(Report) onWarnUser;
  final Function(Report) onContactUser;
